...
def main():
    ...
    # List of threads counts to test.
    ...
    try:
        num_runs = int(input("Enter the number of runs per test file per threads count: "))
    except ValueError:
    ...
    results_dir    = os.path.join("results", "omp")
    ...
    # Each element is a dictionary: { "test_file": ..., "threads": ..., "run": ..., "time": ..., "match": ... }
    results = []

    # Loop over each test file, each threads count, and perform the desired number of runs.
    for test_file in test_files:
        ...
        for t in threads:
            for run in range(1, num_runs + 1):
                print("========================================")
                print(f"Submitting job for test file '{test_file}', threads = {t} (run {run}/{num_runs})...")

                # Define job-specific file names.
                condor_file = f"job_omp_{test_base}_p{t}_{run}.sub"
                out_file    = os.path.join(logs_dir, f"out_omp_{test_base}_p{t}_{run}.txt")
                res_file    = os.path.join(logs_dir, f"res_omp_{test_base}_p{t}_{run}.txt")
                log_file    = os.path.join(logs_dir, f"log_omp_{test_base}_p{t}_{run}.txt")
                err_file    = os.path.join(logs_dir, f"err_omp_{test_base}_p{t}_{run}.txt")
...
arguments = {input_path} {cluster} {ITERATIONS} {CHANGES} {THRESHOLD} {res_file} {t}
request_cpus = 64
getenv = True
queue
"""
                ...
                try:
                    ...
                    print(f"Error submitting job (run {run} for {test_file} with {t} threads): {e}")
                    continue
                ...
                if time_value is not None:
                    print(f"Run {run} for '{test_file}' with {t} threads: Computation time = {time_value} seconds")
                else:
    ...
    # ---------------------------
    raw_csv_file = "computation_times_omp.csv"
    try:
        with open(raw_csv_file, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(["Test File", "Threads", "Run", "Computation Time (s)", "Matches Sequential"])
            for row in results:
                writer.writerow([row["test_file"], row["threads"], row["run"], row["time"],
    ...
    # ---------------------------
    # Compute and display summary statistics for each test file and thread count.
    # These include the minimum, Q1, median, Q3, maximum, average, std deviation,
    # and whether the outputs match the sequential version.
    # ---------------------------
    # We group by a (test_file, threads) tuple.
    summary = {}  # key = (test_file, threads), value = list of run dictionaries
    for row in results:
        key = (row["test_file"], row["threads"])
        summary.setdefault(key, []).append(row)

    summary_csv_file = "summary_statistics_omp.csv"
    try:
        with open(summary_csv_file, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            # Added extra column "Matches Sequential"
            writer.writerow(["Test File", "Threads", "Min", "Q1", "Median", "Q3", "Max",
                             "Average", "Std Dev", "Matches Sequential"])
            print("\nSummary statistics per test file and threads count:")
            for (test_file, t), run_list in summary.items():
