...
efficiency_data = []
for threads, test_file in scaling_threads_pairs.items():
    ...
    parallel_time = omp_mpi_data[(omp_mpi_data['Test File'] == test_file) & 
                             (omp_mpi_data['Processes'] == num_processes) & 
                             (omp_mpi_data['Threads'] == threads)]['Average'].values[0]
    ...
    std_dev = omp_mpi_data[(omp_mpi_data['Test File'] == test_file) & 
                       (omp_mpi_data['Processes'] == num_processes) & 
                       (omp_mpi_data['Threads'] == threads)]['Std Dev'].values[0]
    
    # Calculate efficiency
    efficiency = serial_time / (num_processes * threads * parallel_time)
...
efficiency_df['Scaled Std Dev'] = efficiency_df['Std Dev'] / 20
...
# Customize the plot
plt.title(f'Weak Scaling, Fixed MPI Processes = {num_processes}')
...
plt.savefig('/Users/davide/Desktop/Parallel-KMeans/Plots/mpi_omp/weak_scaling_proc_omp_mpi_4.png', dpi=150)
